"""Seed the single-row config table from a gitignored local file.

author_birthdate is the one private project constant (it drives personal-time math). It
lives in src/data/local/config.toml, which is gitignored and never committed (ADR-004),
so a rebuild restores it on this machine without putting PII in Git. On a fresh clone
the file is absent and this step is simply skipped.

    src/data/local/config.toml:
