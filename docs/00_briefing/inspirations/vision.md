...
## More than a book: a system

This is about more than producing a printed book from years of personal writing.
The aim is to build a durable autobiographical system capable of evolving across an
entire lifetime.
...
depending on size and printing. The cadence is chosen at render time and is not a property of
...
Beyond telling a life, the priority is to build an archive that is:
...
- **Reproducible for any date range.** Any volume is reconstructable from the data with
  the same scripted (not manual) process. All volumes share one system instead of bespoke
...
- **Privacy is a first-class feature.** Sensitive content is modeled in the data
  ...
  goal is informed reader consent rather than censorship.
